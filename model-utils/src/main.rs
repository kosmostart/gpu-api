...
fn main() {
    env_logger::init();
    process_model("box", "box", "box", "glb");
}

fn process_model(model_name: &str, folder_name: &str, file_name: &str, extension: &str) {    
    let (model_data, _loaded_images) = load(model_name, &format!("../models/{}/{}.{}", folder_name, file_name, extension), true, false, vec![]);
